...
if (!isset($conn) || !($conn instanceof mysqli)) {
    die("Database connection not properly initialized. Please check config.php.");
}
if ($conn->connect_errno) {
    die("Database connection failed: " . $conn->connect_error);
}

// ✅ Use discount_applications table for PWD category
$query = "SELECT * FROM discount_applications WHERE UserID = ? AND Category = 'PWD' ORDER BY SubmittedAt DESC LIMIT 1";
...
<?php if ($hasApplication): ?>
  ...
  <?php if ($status === 'Approved'): ?>
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-green-600">
        <h1 class="text-3xl font-bold text-green-700 mb-4">♿ Verified PWD Discount!</h1>
        <p class="text-gray-700 mb-6">Congratulations! Your PWD verification has been <span class="font-semibold text-green-700">approved</span>.</p>
        <div class="bg-green-50 border border-green-300 rounded-lg p-6 mb-6">
            <h2 class="text-lg font-semibold text-green-700 mb-2">You are now eligible for PWD fare discounts.</h2>
  ...
  <?php elseif ($status === 'Rejected'): ?>
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-red-600">
        ...
        <p class="text-gray-700 mb-4">Unfortunately, your PWD discount application has been rejected.</p>
        ...
        ?>
          <form action="reapply_pwd.php" method="POST">
    ...
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-yellow-500">
        ...
        <div class="bg-yellow-50 border border-yellow-300 rounded-lg p-6 mb-4 text-left">
            ...
            <ul class="list-disc list-inside text-gray-700">
                <li><a href="../uploads/<?= htmlspecialchars($application['ID_Front']) ?>" target="_blank">PWD ID (Front)</a></li>
                <li>PWD ID (Back)</li>
...
<?php else: ?>
  <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-3xl">
      <h1 class="text-2xl font-bold text-center text-green-800 mb-3">PWD VERIFICATION</h1>
      ...
      <form action="upload_pwd.php" method="POST" enctype="multipart/form-data" class="flex flex-col flex-grow">
        <!-- STEP 1 -->
        <div id="step1" class="step" style="display:block;">
          <h2 class="font-semibold text-lg mb-3">Required Documents</h2>
          <p class="text-gray-700 mb-6">Upload your PWD ID (Front & Back)</p>

          <div class="grid grid-cols-2 gap-6 mb-6">
            <div class="border border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center">
              <label class="font-medium mb-2">PWD ID (Front) <span class="text-red-500">*</span></label>
            ...
            <div class="border border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center">
              <label class="font-medium mb-2">PWD ID (Back) <span class="text-red-500">*</span></label>
        ...
        <div id="step2" class="step" style="display:none;">
          <h2 class="font-semibold text-lg mb-3">Additional Document</h2>
          <div class="border border-gray-300 rounded-lg p-6 mb-6 flex flex-col items-center justify-center">
            <label class="font-medium mb-2">Upload Medical Certificate (or Barangay Certification)</label>
            <img id="previewCert" class="hidden w-48 h-48 object-cover mb-3 border rounded" />
            <input type="file" name="medicalCert" class="border p-2 rounded w-full" id="medicalCert" accept="image/*,application/pdf" onchange="previewImage(event, 'previewCert')" required>
          </div>
        ...
        <div id="step3" class="step" style="display:none;">
          ...
          <div class="space-y-3">
            <input type="text" name="fullName" placeholder="Full Name" class="border rounded w-full p-2" required>
            <input type="email" name="email" placeholder="Email" class="border rounded w-full p-2" required>
            <input type="text" name="disabilityType" placeholder="Type of Disability" class="border rounded w-full p-2" required>
