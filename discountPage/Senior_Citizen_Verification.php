...
<header>
  <div class="header-left">
    ...
    <span class="page-title">sENIOR CITIZEN VERIFICATION</span> <!-- or App Name -->
  </div>
...
<?php if ($hasApplication): ?>
  ...
  <?php if ($status === 'Approved'): ?>
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-green-600">
        <h1 class="text-3xl font-bold text-green-700 mb-4">🧓 Verified Senior Citizen Discount!</h1>
        <p class="text-gray-700 mb-6">Congratulations! Your senior citizen verification has been <span class="font-semibold text-green-700">approved</span>.</p>
        <div class="bg-green-50 border border-green-300 rounded-lg p-6 mb-6">
            <h2 class="text-lg font-semibold text-green-700 mb-2">You are now eligible for senior fare discounts.</h2>
  ...
  <?php elseif ($status === 'Rejected'): ?>
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-red-600">
        ...
        <p class="text-gray-700 mb-4">Unfortunately, your senior discount application has been rejected.</p>
        ...
        ?>
          <form action="reapply_senior.php" method="POST">
    ...
    <div class="bg-white shadow-lg rounded-2xl p-10 w-full max-w-2xl text-center border-t-8 border-yellow-500">
        ...
        <div class="bg-yellow-50 border border-yellow-300 rounded-lg p-6 mb-4 text-left">
            ...
            <ul class="list-disc list-inside text-gray-700">
                <li><a href="../uploads/<?= htmlspecialchars($application['ID_Front']) ?>" target="_blank">Senior Citizen ID (Front)</a></li>
                <li>Senior Citizen ID (Back)</li>
                <li>Proof of Age or Residency</li>
            </ul>
        </div>
...
<?php else: ?>
  ...
  <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-3xl">
      <h1 class="text-2xl font-bold text-center text-green-800 mb-3">SENIOR CITIZEN VERIFICATION</h1>
      ...
      <form action="upload_senior.php" method="POST" enctype="multipart/form-data" class="flex flex-col flex-grow">
        <!-- STEP 1 -->
        <div id="step1" class="step" style="display:block;">
          <h2 class="font-semibold text-lg mb-3">Required Documents</h2>
          <p class="text-gray-700 mb-6">Upload your Senior Citizen ID (Front & Back)</p>

          <div class="grid grid-cols-2 gap-6 mb-6">
            <div class="border border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center">
              <label class="font-medium mb-2"> Senior ID (Front) <span class="text-red-500">*</span></label>
            ...
            <div class="border border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center">
              <label class="font-medium mb-2"> Senior ID (Back) <span class="text-red-500">*</span></label>
        ...
        <div id="step2" class="step" style="display:none;">
          ...
          <div class="border border-gray-300 rounded-lg p-6 mb-6 flex flex-col items-center justify-center">
            <label class="font-medium mb-2">Upload Proof of Age or Residency</label>
            <img id="previewProof" class="hidden w-48 h-48 object-cover mb-3 border rounded" />
            <input type="file" name="proofDocument" class="border p-2 rounded w-full" id="proofDocument" accept="image/*,application/pdf" onchange="previewImage(event, 'previewProof')" required>
          </div>
