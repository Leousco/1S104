    ...
    if ($result->num_rows === 1) { 
        ...
        if (password_verify($password, $user['PasswordHash'])) { 
            ...
            if ($_SESSION['login_attempts'] >= 5) { 
                $_SESSION['ban_time'] = time() + (1* 60); // 3 minutes ban 
                $_SESSION['login_attempts'] = 0; 
                $banRemaining = 1 * 60; 
                $message = "Too many failed attempts. You are banned for 1 minutes."; 
            } else { 
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  form { padding: 20px 30px 30px 30px; }
...
<div id="error-message" class="form-error-centered">
  ...
  <input type="email" id="login-email" name="email" placeholder="Enter email" required value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>" />
...
<div id="ban-countdown" style="text-align:center; color:#cc0000; font-weight:bold;"></div>
